...
describe('Twitter', () => {
    ...
    test('login', async () => {
        ...
        twitter = await twitter_1.Twitter.login({
            username,
            password,
            otpSecret,
            puppeteerOptions: {
    ...
    test('getHomeTimelineRawTweets', async () => {
        const response = await twitter.getHomeTimelineRawTweets({
            timelineType: options_1.TimelineType.RECOMMEND,
        });
        expect(response).toBeTruthy();
    ...
    test('getHomeTimelineTweets:following', async () => {
        const response = await twitter.getHomeTimelineTweets({
            timelineType: options_1.TimelineType.FOLLOWING,
        });
        expect(response).toBeTruthy();
        expect(response.length).toBeGreaterThan(0);
    });
    ...
    test('searchTweets:live(limit100)', async () => {
        const response = await twitter.searchTweets({
            query: 'from:book000',
            searchType: options_1.SearchType.LIVE,
            limit: 100,
        });
        expect(response).toBeTruthy();
        expect(response.length).toBeGreaterThan(0);
    ...
    test('searchTweets:image', async () => {
        const response = await twitter.searchTweets({
            query: 'from:book000',
            searchType: options_1.SearchType.IMAGE,
        });
        expect(response).toBeTruthy();
        expect(response.length).toBeGreaterThan(0);
    });
    test('searchTweets:video', async () => {
        const response = await twitter.searchTweets({
            query: 'from:book000',
            searchType: options_1.SearchType.VIDEO,
        });
        expect(response).toBeTruthy();
        expect(response.length).toBeGreaterThan(0);
    });
    ...
    test('likeTweet', async () => {
        await expect(twitter.likeTweet({
            tweetId: '1685608131618086912',
        })).resolves.toBe('Done');
    });
    test('unlikeTweet', async () => {
        await expect(twitter.unlikeTweet({
            tweetId: '1685608131618086912',
        })).resolves.toBe('Done');
    });
    test('blockUser', async () => {
        await expect(twitter.blockUser({
            screenName: 'X',
        })).resolves.not.toThrow(exceptions_1.TwitterOperationError);
    });
    test('unblockUser', async () => {
        await expect(twitter.unblockUser({
            screenName: 'X',
        })).resolves.not.toThrow(exceptions_1.TwitterOperationError);
    });
    afterAll(async () => {
