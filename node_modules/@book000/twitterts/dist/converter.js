...
exports.ObjectConverter = {
     ...
     * @returns 拡張エンティティ
     */
    convertExtendedEntities(legacy) {
    ...
    convertPlace(legacy) {
        ...
        return {
            ...
            bounding_box: {
                coordinates: legacy.place.bounding_box.coordinates.map((coordinate) => coordinate.map((coordinate) => [coordinate[1], coordinate[0]])),
                type: legacy.place.bounding_box.type,
            },
            contained_within: legacy.place.contained_within,
    ...
    convertToStatus(tweet) {
        ...
        if (!legacy) {
            throw new exceptions_1.ResponseParseError('Failed to get legacy');
        }
        const userResult = tweet.core?.user_results.result ??
            tweet.tweet?.core?.user_results.result ??
            undefined;
        if (!userResult) {
            throw new exceptions_1.ResponseParseError('Failed to get userResult');
        }
        return {
            ...
            display_text_range: legacy.display_text_range
                ...
                : undefined,
            entities: exports.ObjectConverter.convertTweetEntities(legacy),
            extended_entities: exports.ObjectConverter.convertExtendedEntities(legacy),
            coordinates: exports.ObjectConverter.convertCoordinates(legacy),
            place: exports.ObjectConverter.convertPlace(legacy),
        };
    },
    /**
